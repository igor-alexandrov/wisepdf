  ...
  module Render
    ...
    def render(options = nil, *args, &block)
      if options.is_a?(Hash) && options.has_key?(:pdf)
        ...
        make_and_send_pdf(options.delete(:pdf), Pdf::Configuration.options.merge(options))
      else
        super
      end
    end
    ...
    def render_to_string(options = nil, *args, &block)
      if options.is_a?(Hash) && options.has_key?(:pdf)
        ...
        options[:basic_auth] = set_basic_auth(options)
        options.delete :pdf
        make_pdf(Pdf::Configuration.options.merge(options))
      else
        super
      end
    end
    ...
    private
      ...
      def set_basic_auth(options={})
        options[:basic_auth] ||= Pdf::Configuration.options.fetch(:basic_auth){ false }
      ...
      def make_pdf(options = {})
        ...
        w = Pdf::Writer.new(options[:wkhtmltopdf])
      ...
      def prerender_header_and_footer(options)
        [:header, :footer].each do |hf|
          if options[hf] && options[hf][:html] && options[hf][:html][:template]
            ...
            @hf_tempfiles.push( tf = Pdf::Tempfile.new("wicked_#{hf}_pdf.html") )
